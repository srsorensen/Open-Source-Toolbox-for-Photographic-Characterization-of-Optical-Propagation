...
def load_data(directory,contains,f_ending,threshold,alpha_threshold,r_squared_threshold):
    ...
    for file in os.listdir(directory):
        if contains.lower() in file.lower() and f_ending.lower() in file.lower():
            ...
            hf.close()
    return TE_data, TE_wav, TE_weights, TM_data, TM_wav, TE_weights

directory = 'E:/Top_Down_Method/Article_Data/Processed data/AlGaAs/Correct_Optimized/'
contains = 'ST3'
f_ending = '.h5'
threshold = False
...
for i in range(8):
    print('data', i ,len(TM_data[i]))
    print('wav', i , len(TM_wav[i]))
    print('weights', i, len(TM_weights[i]))
...
print(np.round(te_mean[910],1),np.round(te_std[910],1))
print(np.round(te_mean[945],1),np.round(te_std[945],1))
print(np.round(te_mean[980],1),np.round(te_std[980],1))
